<div align="center">

<h1>🚀 HaxScipt Client</h1>

<p>
  <img src="https://img.shields.io/badge/SON%20SÜRÜM-v1.0-ffd700?style=for-the-badge&labelColor=black" alt="Version">
  <img src="https://img.shields.io/badge/DURUM-AKTİF-2ea44f?style=for-the-badge&labelColor=black" alt="Status">
  <img src="https://img.shields.io/badge/PLATFORM-WIN%20%7C%20MAC%20%7C%20LINUX-00e5ff?style=for-the-badge&labelColor=black" alt="Platform">
  <a href="https://discord.gg/haxscipt">
    <img src="https://img.shields.io/badge/DISCORD-KATIL-5865F2?style=for-the-badge&logo=discord&logoColor=white&labelColor=black" alt="Discord">
  </a>
</p>

<br>
<h3>
  Real Soccer ve Haxball dünyası için geliştirilmiş;<br>
  🇹🇷 <b>Türkiye'de İlk Chrome Flags Entegrasyonuna Sahip</b><br>
  profesyonel masaüstü istemcisi.
</h3>

<p>
  <i>Minimum Input Lag • Stabil FPS • İnce Çizgi Modu • Hile Koruması</i>
</p>

<br>

<img src="https://i.imgur.com/QPvaf2g.png" width="100%" style="border-radius: 12px; box-shadow: 0 0 25px rgba(0,0,0,0.6); border: 1px solid rgba(255,255,255,0.1);">

</div>

<br>

---

## 🔥 Neden HaxScipt Client?

Bu proje, standart tarayıcıların hantallığından kurtulup, **Real Soccer** sunucularında rekabetçi avantaj sağlamak isteyen oyuncular için sıfırdan tasarlandı.

### ⚡ Performans & Optimizasyon (YENİ)
> **"Türkiye'de Bir İlk"**
* **🛠 Gömülü Chrome Flags Yöneticisi:** Tarayıcı derinliklerindeki ayarları (flags) client arayüzüne taşıdık. Kendi donanımına göre input lag'ı en dibe çekebilirsin.
* **🚀 Optimize Chromium Yapısı:** Gereksiz tüm servisler kapatıldı. Minimum RAM kullanımı, maksimum işlemci odaklanması.
* **📏 İnce Çizgi Modu (Thin Line):** Belki de dünyada tek! **`J`** tuşuna basarak veya ayarlardan açarak oyun içi çizgileri inceltebilirsin. Bu sayede hem FPS artar hem de top kontrolü keskinleşir.

### 🧿 Temel Özellikler
* **🟢 HaxScipt Hızlı Erişim:** Odaları listede aramaya son. Panelden tek tıkla HaxScipt sunucularına en hızlı yoldan bağlan.
* **🔗 Gelişmiş Discord RPC:** Oyundaki durumun (Skor, Oda Bilgisi) Discord profilinde anlık görünür.
* **🛡️ Güvenlik Kalkanı:** Geliştirilmiş hile ve zararlı eklenti engelleme sistemi ile adil oyun.
* **🎨 Kişiselleştirme:** Karanlık mod, özel temalar ve profil adlandırma sistemi.

---

## 📥 İndirme ve Kurulum

En güncel sürümü **Releases** sayfasından indirebilirsiniz. Windows, macOS ve Linux desteklenmektedir.

<div align="center">
  <br>
  <a href="https://github.com/haxscipt/haxscipt-client/releases">
    <img src="https://img.shields.io/badge/⬇️-CLIENT'I%20İNDİR-ffd700?style=for-the-badge&labelColor=black" height="50">
  </a>
  <br><br>
</div>

---

## ❓ Sık Sorulan Sorular (SSS)

| Soru | Cevap |
| :--- | :--- |
| **İnce Çizgi (Thin Line) nasıl açılır?** | Oyun içindeyken **`J`** tuşuna basmanız veya Ayarlar menüsünden aktif etmeniz yeterlidir. |
| **FPS'im artar mı?** | Evet. Chrome Flags ayarları ve gereksiz servislerin kapatılmasıyla standart tarayıcılara göre ciddi FPS artışı sağlanır. |
| **Client güvenli mi?** | Tamamen güvenlidir. Kodlar açıktır ve GitHub üzerinden checksum doğrulaması yapabilirsiniz. |
| **Oda paneli çalışmıyor?** | Oda listesi sayfasına giriş yaptığınızda veriler otomatik olarak çekilecektir. |

---

## 💜 Destek & Topluluk

Bu proje **HaxScipt** topluluğu için geliştirilmektedir. Bize destek olmak için repoya **STAR** ⭐️ bırakmayı unutmayın!

<div align="center">
  <a href="https://discord.gg/haxscipt">
    <img src="https://img.shields.io/badge/💬-Discord'a%20Katıl-5865F2?style=for-the-badge&logo=discord&logoColor=white&labelColor=black">
  </a>
  <a href="mailto:haxscipt@gmail.com">
    <img src="https://img.shields.io/badge/📧-İletişim-red?style=for-the-badge&labelColor=black">
  </a>
</div>

---

<div align="center">
  <p>
    <a href="https://star-history.com/#haxscipt/haxscipt-client&Date">
     <img src="https://api.star-history.com/svg?repos=haxscipt/haxscipt-client&type=Date" alt="Star History Chart">
    </a>
  </p>
  <br>
  <img src="https://img.shields.io/badge/📜-MIT%20Lisansı-lightgrey?style=for-the-badge&labelColor=black">
  <br>
  <small>© 2026 HaxScipt. Haxball, Mario Carbajal'ın tescilli markasıdır.</small>
</div>
